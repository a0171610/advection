...
contains
  ...
  subroutine mass_correct(f, fold, gmax, gmin, w)
		...
		real(8), intent(in) :: fold(:, :), gmax(:, :), gmin(:, :), w(:, :)
		real(8), allocatable, dimension(:, :) :: f1(:, :), f2(:, :), f3(:, :)
		real(8) :: df, a, b, s2, s3, s2r, f1f2w, f1f1f2f2w, s3s2f3, b1, b2, f1f2!, e
		integer(8) :: nx, ny, i, j

		nx = size(f, 1)
		ny = size(f, 2)
		allocate(f1(nx, ny), f2(nx, ny), f3(nx, ny))
		df = 0.0d0
