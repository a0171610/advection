...
for i in range(n):
  if ((0.1 > G[i] or G[i] < 1.0) and (0.1 > G1[i] or G1[i] > 1.0)):
...
plt.plot(G, G2, color="black")
plt.plot([min(G), max(G)], [max(G2), max(G2)], color="black")
plt.plot([max(G), max(G)], [max(G2), min(G2)], color="black")
plt.plot(G, G3, color="black")
plt.text(1.1, 0.4, "l2_over = {}".format(l2_over), fontsize = 10)
plt.text(1.1, 0.3, "l2_real = {}".format(l2_real), fontsize = 10)
plt.text(1.1, 0.2, "l2_shape = {}".format(l2_shape), fontsize = 10)
